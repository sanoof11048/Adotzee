...
export default function Commerce() {
  ...
  const typedCommerceCourses: Course[] = commerceCourses;
  ...
  const filteredCourses: Course[] = searchTerm.trim()
    ? typedCommerceCourses
        .map((c) => ({
          ...c,
  ...
  const highlightText = (text: string, keyword: string) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === keyword.toLowerCase() ? (
        <span key={i} className="bg-yellow-300/30 text-white font-bold px-1 rounded">
          {part}
        </span>
      ) : (
        part
            ...
            <div key={index} className="mb-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <p className="font-semibold text-md md:text-lg">{course.category}</p>
