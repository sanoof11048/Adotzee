...
export default function Humanities(): JSX.Element {
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [selectedCategory, setSelectedCategory] = useState<string>("");
  const [showSearch, setShowSearch] = useState<boolean>(false);

  const navigate = useNavigate();
  const { humanitiesCourses } = useCourse() as {
    humanitiesCourses: HumanitiesCourse[];
  };
  ...
  const filteredCourses: HumanitiesCourse[] = searchTerm.trim()
    ? humanitiesCourses
        .map((c) => ({
          ...c,
          addons: c.addons.filter(
            (a) =>
              a.name &&
  ...
  const highlightText = (
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === keyword.toLowerCase() ? (
        <span
          key={i}
          className="bg-yellow-300/30 text-white font-bold px-1 rounded"
        >
          {part}
        </span>
      ) : (
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-linear-to-br from-[#6a85b6] to-[#bac8e0] pt-10 px-4 pb-10">
        {/* Hero */}
        <div className="text-center text-white mb-8">
          <h1 className="text-3xl md:text-4xl mt-20 md:mt-15 font-bold">
            Explore Humanities Career Options
          </h1>
          <p className="mt-2 text-lg flex justify-center items-center">
            <FontAwesomeIcon icon={faGraduationCap} className="mr-2" />
            Over{" "}
            <span className="text-yellow-400 font-bold mx-1">24+</span>{" "}
            students started careers with us!
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-primary/50 backdrop-blur-lg p-6 rounded-xl shadow-xl mb-10">
          ...
          {searchTerm && (
            <div className="mb-4 flex justify-between items-center bg-white/20 p-3 rounded-md">
              ...
              <button
                ...
                className="text-white hover:text-gray-600 bg-black/10 rounded-md"
              >
          ...
          {filteredCourses.map((course, index) => (
            <div key={index} className="mb-4">
              <div
                className={`cursor-pointer p-4 border border-white/40 rounded-lg shadow-md flex justify-between items-center ${
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-white/20 px-2 rounded-full text-sm whitespace-nowrap overflow-hidden text-ellipsis">
                    {course.addons.length} courses
                  </span>
                </div>
                <FontAwesomeIcon
                  icon={
                      ...
                      >
                        <span className="flex items-center">
                          ...
                          <p className="font-semibold text-sm md:text-md">
                            {searchTerm
                              ? highlightText(addon.name, searchTerm)
                              : addon.name}
                          </p>
                        </span>
